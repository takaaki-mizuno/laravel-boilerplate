...
@section('breadcrumb')
    <li><a href="{!! action('Admin\UserNotificationController@index') !!}"><i class="fa fa-files-o"></i> UserNotifications</a></li>
    @if( $isNew )
...
@section('content')
    @if( $isNew )
        <form action="{!! action('Admin\UserNotificationController@store') !!}" method="POST" enctype="multipart/form-data">
    @else
        <form action="{!! action('Admin\UserNotificationController@update', [$userNotification->id]) !!}" method="POST" enctype="multipart/form-data">
                ...
                <div class="box-body">
                    ...
                    <select name="locale" class="form-control">
                        @foreach( config('locale.languages') as $code => $locale )
                            <option value="{!! $code !!}" @if( (old('locale') && old('locale') == $code) || (!old('locale') && $userNotification->locale == $code)) selected @endif >@lang(array_get($locale, 'name', $code))</option>
                        @endforeach
                    </select>
                    ...
                    <div class="form-group @if ($errors->has('data')) has-error @endif">
                        ...
                        <textarea name="data" class="form-control" rows="5" placeholder="JSON DATA">{{ old('data') ? old('data') : json_encode($userNotification->data) }}</textarea>
                    </div>

                    <div class="form-group @if ($errors->has('content')) has-error @endif">
                        <label for="content">@lang('admin.pages.user-notifications.columns.content')</label>
                        <textarea name="content" class="form-control" rows="5" placeholder="Content">{{ old('content') ? old('content') : $userNotification->content }}</textarea>
                    </div>
                </div>
